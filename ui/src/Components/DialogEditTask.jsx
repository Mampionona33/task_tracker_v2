...
const DialogEditTask = (props) => {
  ...
  }, [listStatIvpn, listTaskTypes, listStatCom, listTaskCase, selectedRowData,allTaskType]);
  ...
  const handleClickSave = async () => {
    ...
    let taskRefGoal = 0;
    if(allTaskType){      
      taskRef = allTaskType.filter((task) => task.name === refTaskType.current.children[0].children[1].children[0].value);
      taskRefGoal = taskRef[0].objectif;
    }


    const a = calculProd(taskRefGoal,elapstedTime,refNumbrAfter.current.children[1].children[0].value)
